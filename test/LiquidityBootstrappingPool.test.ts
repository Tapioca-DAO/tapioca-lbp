...
import VaultDeployer from '../helpers/models/vault/VaultDeployer';
...
describe('LiquidityBootstrappingPool', function () {
    ...
    context('with invalid creation parameters', () => {
        ...
        it('fails with < 2 tokens', async () => {
            ...
            await expect(WeightedPool.create(params)).to.be.revertedWith(
                'MIN_TOKENS',
            );
        });
            ...
            context(`with ${numTokens} tokens`, () => {
                ...
                it('sets token weights', async () => {
                    ...
                    for (let i = 0; i < numTokens; i++) {
                        expect(normalizedWeights[i]).to.equalWithError(
                            pool.normalizedWeights[i],
                            0.001,
                        );
                    }
                });
                ...
                it('sets scaling factors', async () => {
                    ...
                    const tokenScalingFactors = tokens.map((token) =>
                        fp(10 ** (18 - token.decimals)),
                    );

                    expect(poolScalingFactors).to.deep.equal(
                        tokenScalingFactors,
                    );
                });
            });
    ...
    context('when deployed from factory', () => {
        sharedBeforeEach('deploy pool', async () => {
            const params = {
                tokens,
                weights,
                poolType: WeightedPoolType.LIQUIDIFTY_BOOTSTRAPPING_POOL,
                fromFactory: true,
            };
    ...
    describe('with valid creation parameters', () => {
        context('when initialized with swaps disabled', () => {
            ...
            it('swaps are blocked', async () => {
                await expect(
                    pool.swapGivenIn({in: 1, out: 0, amount: fp(0.1)}),
        ...
        context('when initialized with swaps enabled', () => {
            ...
            it('swaps are not blocked', async () => {
                ...
                await expect(pool.swapGivenIn({in: 1, out: 0, amount: fp(0.1)}))
                    .to.not.be.reverted;
            });
            ...
            it('sets token weights', async () => {
                ...
                expect(normalizedWeights).to.equalWithError(
                    pool.normalizedWeights,
                    0.001,
                );
            });

            it('stores the initial weights as a zero duration weight change', async () => {
                const {startTime, endTime, endWeights} =
                    await pool.getGradualWeightUpdateParams();

                expect(startTime).to.equal(endTime);
                expect(endWeights).to.equalWithError(
                    pool.normalizedWeights,
                    0.001,
                );
            });
            ...
            describe('permissioned actions', () => {
                context('when the sender is the owner', () => {
                    ...
                    it('swaps can be enabled and disabled', async () => {
                        ...
                        expect(await pool.instance.getSwapEnabled()).to.be
                            .false;
                    ...
                    it('disabling swaps emits an event', async () => {
                        const receipt = await pool.setSwapEnabled(
                            sender,
                            false,
                        );
                    ...
                    it('owner can join and receive BPT, then exit', async () => {
                        const bptBeforeJoin = await pool.balanceOf(
                            owner.address,
                        );
                        await expect(
                            pool.joinGivenIn({
                        ...
                        const bptAfterJoin = await pool.balanceOf(
                            owner.address,
                        );
                        ...
                        await expect(
                            pool.exitGivenOut({
                                from: owner,
                                amountsOut: initialBalances,
                            }),
                        ).to.not.be.reverted;
                        const bptAfterExit = await pool.balanceOf(
                            owner.address,
                        );
                    ...
                    describe('update weights gradually', () => {
                        ...
                        context('with invalid parameters', () => {
                            ...
                            it('fails with invalid normalized end weights', async () => {
                                const badWeights = Array(weights.length).fill(
                                    fp(0.6),
                                );
                        ...
                        context(
                            'with valid parameters (ongoing weight update)',
                            () => {
                                const endWeights = [
                                    fp(0.15),
                                    fp(0.25),
                                ...
                                it('stores the params', async () => {
                                    ...
                                    ).to.equalWithError(startTime, 0.001);
                                    expect(
                                        updateParams.endTime,
                                    ).to.equalWithError(endTime, 0.001);
                                    expect(
                                        updateParams.endWeights,
                                    ).to.equalWithError(endWeights, 0.001);
                                });
                            },
                        );
                    });
                ...
                context('when the sender is not the owner', () => {
                    it('non-owner cannot initialize the pool', async () => {
                        await expect(
                            pool.init({from: other, initialBalances}),
                        ).to.be.revertedWith('CALLER_IS_NOT_LBP_OWNER');
                    });
