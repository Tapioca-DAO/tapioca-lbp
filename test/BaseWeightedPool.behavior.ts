...
import { BigNumberish, bn, fp, fpMul, pct } from '../helpers/numbers';
...
import {
    ...
    WeightedPoolType,
} from '../helpers/models/pools/weighted/types';
import { ZERO_ADDRESS } from '../helpers/constants';
import { sharedBeforeEach } from '../common/sharedBeforeEach';
import { expectBalanceChange } from '../helpers/test/tokenBalance';
...
export function itBehavesAsWeightedPool(
    ...
    async function deployPool(
        params: RawWeightedPoolDeployment = {},
    ): Promise<void> {
    ...
    describe('creation', () => {
        context('when the creation succeeds', () => {
            ...
            it('uses the corresponding specialization', async () => {
                ...
                const { address, specialization } =
                    await pool.getRegisteredInfo();
        ...
        context('when the creation fails', () => {
            it('reverts if the number of tokens and weights do not match', async () => {
                ...
                await expect(
                    deployPool({ weights: badWeights }),
                ).to.be.revertedWith('INPUT_LENGTH_MISMATCH');
            });

            it('reverts if there are repeated tokens', async () => {
                const badTokens = new TokenList(
                    Array(numberOfTokens).fill(tokens.first),
                );

                await expect(
                    deployPool({ tokens: badTokens, fromFactory: true }),
            ...
            it('reverts if the swap fee is too high', async () => {
                ...
                await expect(
                    deployPool({ swapFeePercentage: badSwapFeePercentage }),
    ...
    describe('onJoinPool', () => {
        function itJoins() {
            ...
            it('fails if no user data', async () => {
                await expect(pool.join({ data: '0x' })).to.be.revertedWith(
                    'Transaction reverted without a reason',
                );
            });

            it('fails if wrong user data', async () => {
                const wrongUserData = ethers.utils.defaultAbiCoder.encode(
                    ['address'],
                    [lp.address],
                );

                await expect(
                    pool.join({ data: wrongUserData }),
            ...
            context('initialization', () => {
                it('grants the n * invariant amount of BPT', async () => {
                    ...
                    const { amountsIn, dueProtocolFeeAmounts } =
                        await pool.init({
                            recipient,
                            initialBalances,
                            from: lp,
                    ...
                    expect(
                        await pool.balanceOf(recipient),
                    ).to.be.equalWithError(
                        invariant.mul(numberOfTokens),
                        0.001,
                    );
                });
                ...
                it('fails if already initialized', async () => {
                    ...
                    await expect(
                        pool.init({ initialBalances, from: lp }),
            ...
            context('join exact tokens in for BPT out', () => {
                ...
                context('once initialized', () => {
                    ...
                    it('grants BPT for exact tokens', async () => {
                        const previousBptBalance = await pool.balanceOf(
                            recipient,
                        );
                        ...
                        const currentBptBalance = await pool.balanceOf(
                            recipient,
                        );
                        expect(
                            currentBptBalance.sub(previousBptBalance),
                        ).to.be.equalWithError(expectedBptOut, 0.0001);
                    });

                    it('can tell how much BPT it will give in return', async () => {
                        const minimumBptOut = pct(expectedBptOut, 0.99);

                        const result = await pool.queryJoinGivenIn({
                            amountsIn,
                            minimumBptOut,
                        });

                        expect(result.amountsIn).to.deep.equal(amountsIn);
                        expect(result.bptOut).to.be.equalWithError(
                            expectedBptOut,
                            0.0001,
                        );
                    });
                    ...
                    it('fails if not enough BPT', async () => {
                        ...
                        await expect(
                            pool.joinGivenIn({ amountsIn, minimumBptOut }),
            ...
            context('join token in for exact BPT out', () => {
                ...
                it('fails if not initialized', async () => {
                    await expect(
                        pool.joinGivenOut({ bptOut, token }),
                    ).to.be.revertedWith('UNINITIALIZED');
                });
                ...
                context('once initialized', () => {
                    ...
                    it('grants exact BPT for token in', async () => {
                        ...
                        const expectedAmountIn = await pool.estimateTokenIn(
                            token,
                            bptOut,
                            initialBalances,
                        );

                        const result = await pool.joinGivenOut({
                            recipient,
                            bptOut,
                            token,
                            from: lp,
                        ...
                        expect(result.amountsIn[token]).to.be.equalWithError(
                            expectedAmountIn,
                            0.001,
                        );
                        expect(result.amountsIn.filter((_, i) => i != token)).to
                            .be.zeros;
                    ...
                    it('can tell what token amounts it will have to receive', async () => {
                        ...
                        const result = await pool.queryJoinGivenOut({
                            bptOut,
                            token,
                        });
                    ...
                    it('fails if invariant increases more than max allowed', async () => {
                        ...
                        // is more than 3
                        const bptOut = (
                            await pool.getMaxInvariantIncrease()
                        ).add(10);
            ...
            context('join all tokens in for exact BPT out', () => {
                it('fails if not initialized', async () => {
                    await expect(
                        pool.joinAllGivenOut({ bptOut: fp(2) }),
                    ).to.be.revertedWith('UNINITIALIZED');
                });
                ...
                context('once initialized', () => {
                    ...
                    it('grants exact BPT for tokens in', async () => {
                        ...
                        const expectedAmountsIn = initialBalances.map(
                            (balance) => balance.div(2),
                        );

                        const result = await pool.joinAllGivenOut({
                            recipient,
                            bptOut,
                            from: lp,
                        });
                        ...
                        for (let i = 0; i < expectedAmountsIn.length; i++) {
                            expect(result.amountsIn[i]).to.be.equalWithError(
                                expectedAmountsIn[i],
                                0.001,
                            );
                        }
                    ...
                    it('can tell what token amounts it will have to receive', async () => {
                        ...
                        const result = await pool.queryJoinAllGivenOut({
                            bptOut,
                        });
    ...
    describe('onExitPool', () => {
        ...
        function itExits() {
            ...
            it('fails if no user data', async () => {
                await expect(pool.exit({ data: '0x' })).to.be.revertedWith(
                    'Transaction reverted without a reason',
                );
            });
            ...
            context('exit exact BPT in for one token out', () => {
                ...
                it('grants one token for exact bpt', async () => {
                    ...
                    const expectedTokenOut = await pool.estimateTokenOut(
                        token,
                        bptIn,
                    );
                    ...
                    expect(result.amountsOut[token]).to.be.equalWithError(
                        expectedTokenOut,
                        0.0001,
                    );
                    expect(result.amountsOut.filter((_, i) => i != token)).to.be
                        .zeros;

                    // Current BPT balance should decrease
                    expect(await pool.balanceOf(lp)).to.equal(
                        previousBptBalance.sub(bptIn),
                    );
                });
                ...
                it('can tell how many tokens it will give in return', async () => {
                    ...
                    const result = await pool.querySingleExitGivenIn({
                        bptIn,
                        token,
                    });
                ...
                it('fails if invariant decreases more than max allowed', async () => {
                    ...
                    // is more than 0.7
                    const bptIn = (await pool.getMaxInvariantDecrease()).add(
                        10,
                    );
                    await expect(
                        pool.singleExitGivenIn({ bptIn, token }),
            ...
            context('exit exact BPT in for all tokens out', () => {
                it('grants all tokens for exact bpt', async () => {
                    ...
                    expect(result.amountsOut).to.be.equalWithError(
                        expectedAmountsOut,
                        0.001,
                    );
                ...
                it('fully exit', async () => {
                    ...
                    const expectedAmountsOut = initialBalances.map((balance) =>
                        balance.mul(previousBptBalance).div(totalBPT),
                    );

                    const result = await pool.multiExitGivenIn({
                        from: lp,
                        bptIn: previousBptBalance,
                    });
                    ...
                    expect(result.amountsOut).to.be.lteWithError(
                        expectedAmountsOut,
                        0.00001,
                    );
                ...
                it('can tell what token amounts it will give in return', async () => {
                    ...
                    const result = await pool.queryMultiExitGivenIn({
                        bptIn: previousBptBalance,
                    });
                ...
                it('reverts if paused', async () => {
                    ...
                    const bptIn = previousBptBalance.div(2);
                    await expect(
                        pool.multiExitGivenIn({ from: lp, bptIn }),
                    ).to.be.revertedWith('PAUSED');
                });
            });
            ...
            context('exit BPT in for exact tokens out', () => {
                it('grants exact tokens for bpt', async () => {
                    ...
                    const amountsOut = initialBalances.map((balance) =>
                        balance.div(2),
                    );
                ...
                it('can tell how much BPT it will have to receive', async () => {
                    ...
                    const result = await pool.queryExitGivenOut({
                        amountsOut,
                        maximumBptIn,
                    });

                    expect(result.amountsOut).to.deep.equal(amountsOut);
                    expect(result.bptIn).to.be.equalWithError(
                        previousBptBalance.div(2),
                        0.001,
                    );
                });
                ...
                it('fails if more BTP needed', async () => {
                    ...
                    ).to.be.revertedWith('BPT_IN_MAX_AMOUNT');
                });
                ...
                it('reverts if paused', async () => {
                    ...
                    const amountsOut = initialBalances;
                    await expect(
                        pool.exitGivenOut({ from: lp, amountsOut }),
                    ).to.be.revertedWith('PAUSED');
                });
            });
        }
        ...
        function itSwaps() {
            context('given in', () => {
                ...
                it('calculates amount out', async () => {
                    const amount = fp(0.1);
                    const amountWithFees = fpMul(
                        amount,
                        POOL_SWAP_FEE_PERCENTAGE.add(fp(1)),
                    );
                    ...
                    expect(result.amount).to.be.equalWithError(
                        expectedAmountOut,
                        0.01,
                    );
                });
                ...
                it('calculates max amount out', async () => {
                    ...
                    const maxAmountInWithFees = fpMul(
                        maxAmountIn,
                        POOL_SWAP_FEE_PERCENTAGE.add(fp(1)),
                    );
                    const expectedAmountOut = await pool.estimateGivenIn({
                ...
                it('reverts if token in exceeds max in ratio', async () => {
                    ...
                    await expect(
                        pool.swapGivenIn({ in: 1, out: 0, amount, from: lp }),
                    ).to.be.revertedWith('MAX_IN_RATIO');
                });
            ...
            context('given out', () => {
                ...
                it('calculates amount in', async () => {
                    const amount = fp(0.1);
                    const expectedAmountIn = await pool.estimateGivenOut({
                        in: 1,
                        out: 0,
                        amount,
                    });
                ...
                it('reverts if token in exceeds max out ratio', async () => {
                    ...
                    await expect(
                        pool.swapGivenOut({ in: 1, out: 0, amount, from: lp }),
                    ).to.be.revertedWith('MAX_OUT_RATIO');
                });
    ...
    describe('protocol swap fees', () => {
        ...
        context('without balance changes', () => {
            it('no protocol fees on joins and exits', async () => {
                let joinResult = await pool.joinGivenIn({
                    from: lp,
                    amountsIn: fp(100),
                    protocolFeePercentage,
                });
                ...
                joinResult = await pool.joinGivenOut({
                    from: lp,
                    bptOut: fp(1),
                    token: 0,
                    protocolFeePercentage,
                ...
                joinResult = await pool.joinAllGivenOut({
                    from: lp,
                    bptOut: fp(0.1),
                });
                expect(joinResult.dueProtocolFeeAmounts).to.be.zeros;

                let exitResult = await pool.singleExitGivenIn({
                    from: lp,
                    bptIn: fp(10),
                    token: 0,
                    protocolFeePercentage,
                ...
                exitResult = await pool.multiExitGivenIn({
                    from: lp,
                    bptIn: fp(10),
                    protocolFeePercentage,
                });
                ...
                joinResult = await pool.joinGivenIn({
                    from: lp,
                    amountsIn: fp(10),
                    protocolFeePercentage,
                });
                ...
                exitResult = await pool.exitGivenOut({
                    from: lp,
                    amountsOut: fp(10),
                    protocolFeePercentage,
                });
        ...
        context('with balance changes', () => {
            ...
            sharedBeforeEach('simulate doubled initial balances', async () => {
                ...
                currentBalances = initialBalances.map((balance) =>
                    balance.mul(4).div(3),
                );
            });
