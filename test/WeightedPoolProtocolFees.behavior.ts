...
import { WeightedPoolType } from '../helpers/models/pools/weighted/types';
...
import { bn, fp, fpDiv, FP_100_PCT, FP_ZERO } from '../helpers/numbers';
...
export function itPaysProtocolFeesFromInvariantGrowth(): void {
    ...
    describe('invariant growth protocol fees', () => {
        ...
        sharedBeforeEach(async () => {
            ...
            tokens = await TokenList.create(numTokens, {
                sorted: true,
                varyDecimals: true,
            });
            rateProviders = await tokens.asyncMap(() =>
                deploy('v2-pool-utils/MockRateProvider'),
            );

            await vault.setSwapFeePercentage(protocolFeePercentage);
            ({ address: protocolFeesCollector } =
                await vault.getFeesCollector());
        ...
        describe('last post join/exit invariant', () => {
            it('is set on initialization', async () => {
                ...
                expectEqualWithError(
                    await pool.getLastPostJoinExitInvariant(),
                    await pool.estimateInvariant(),
                );
            });
            ...
            context('once initialized and with accumulated fees', () => {
                ...
                sharedBeforeEach(
                    'accumulate fees by increasing balance',
                    async () => {
                        await pool.vault.updateCash(
                            pool.poolId,
                            initialBalances.map((x) =>
                                x.mul(initialBalanceGrowth),
                            ),
                        );
                    },
                );
                ...
                function itIsUpdatedByJoins() {
                    it('is updated by joins', async () => {
                        ...
                        await pool.join({
                            data: WeightedPoolEncoder.joinAllTokensInForExactBPTOut(
                                (await pool.totalSupply()).div(2),
                            ),
                            from: lp,
                        });
                ...
                function itIsUpdatedByExits() {
                    it('is updated by exits', async () => {
                        ...
                        await pool.exit({
                            data: WeightedPoolEncoder.exitExactBPTInForTokensOut(
                                (await pool.totalSupply()).div(2),
                            ),
                            from: lp,
                        });
        ...
        describe('ath rate product', () => {
            context('when the pool is exempt from yield fees', () => {
                ...
                it('is not set on initialization', async () => {
                    ...
                    expect(
                        await yieldFeeExemptPool.instance.getATHRateProduct(),
                    ).to.be.eq(0);
                });
            });
            ...
            context('when the pool pays yield fees', () => {
                it('is set on initialization', async () => {
                    ...
                    const expectedRateProduct = calculateInvariant(
                        rates,
                        pool.weights,
                    );
                    expect(
                        await pool.instance.getATHRateProduct(),
                    ).to.be.almostEqual(expectedRateProduct, 0.0000001);
                });
                        ...
                        sharedBeforeEach(
                            ...
                            async () => {
                                ...
                                for (const [
                                    index,
                                    provider,
                                ] of rateProviders.entries()) {
                                    if (typeof provider !== 'string')
                                        await provider.mockRate(rates[index]);
                                }

                                expectedRateProduct = calculateInvariant(
                                    rates,
                                    pool.weights,
                                );
                            },
        ...
        describe('protocol fees', () => {
            async function protocolFeesPaid(): Promise<BigNumber> {
                const previousProtocolFeeCollectorBalance =
                    await pool.balanceOf(protocolFeesCollector);
                ...
                const currentProtocolFeeCollectorBalance = await pool.balanceOf(
                    protocolFeesCollector,
                );
                return currentProtocolFeeCollectorBalance.sub(
                    previousProtocolFeeCollectorBalance,
                );
            }
            ...
            context('once initialized and with accumulated fees', () => {
                ...
                context('when not paused', () => {
                    it('pays protocol fees', async () => {
                        ...
                        const expectedOwnership = initialBalanceGrowth
                            .sub(1)
                            .mul(protocolFeePercentage)
                            .div(initialBalanceGrowth);

                        await expectEqualWithError(
                            bptOwnership,
                            expectedOwnership,
                        );
                    });
