...
export default {
    // async deploySimple(authorizer:string):Promise<Vault>{
    ...
    async deploy(params: RawVaultDeployment): Promise<Vault> {
        ...
        const authorizerAdaptor = await this._deployAuthorizerAdaptor(
            vault,
            from,
        );
        const adaptorEntrypoint = await this._deployAuthorizerAdaptorEntrypoint(
            authorizerAdaptor,
        );
        ...
        await basicAuthorizer.grantRolesToMany(
    ...
    async _deployBasicAuthorizer(admin: SignerWithAddress): Promise<Contract> {
        return deploy('MockBasicAuthorizer', {args: [], from: admin});
    },
    ...
    async _deployAuthorizer(
        ...
        return deploy('TimelockAuthorizer', {
            args: [admin.address, authorizerAdaptorEntrypoint.address, MONTH],
            from,
        });
    },

    async _deployAuthorizerAdaptor(
        vault: Contract,
        from?: SignerWithAddress,
    ): Promise<Contract> {
        return deploy('AuthorizerAdaptor', {args: [vault.address], from});
    },
    ...
    async _deployAuthorizerAdaptorEntrypoint(
        ...
        return deploy('AuthorizerAdaptorEntrypoint', {
            args: [adaptor.address],
            from,
        });
    },
    ...
    async _deployProtocolFeeProvider(
        ...
        return deploy('ProtocolFeePercentagesProvider', {
