...
class TokensDeployer {
    async deployWeth(sender: SignerWithAddress): Promise<Contract> {
        return await deploy('TestWETH', {
            from: sender,
            args: [],
        });
    }
    async deploy(
        ...
        const trimmedParams = sorted
            ? this._trimParamsForSortedDeploy(params)
            : params;
        const deployments: TokenDeployment[] =
            TypesConverter.toTokenDeployments(
                trimmedParams,
                defaultSender,
                varyDecimals,
            );
        ...
        const sortedTokens = sorted
            ? this._sortTokensDeployment(tokens, params)
            : tokens;
        return new TokenList(sortedTokens);
    }

    async deployToken(params: RawTokenDeployment): Promise<Token> {
        const {symbol, name, decimals, from} =
            TypesConverter.toTokenDeployment(params);
        ...
        if (symbol !== 'WETH') {
            ...
            instance = await this.deployWeth(sender);
        }

        return new Token(name, symbol, decimals, instance);
    }

    private _sortTokensDeployment(
        tokens: Token[],
        params: RawTokensDeployment,
    ): Token[] {
