...
export default class LinearPool extends BasePool {
    ...
    static async create(
        params: RawLinearPoolDeployment,
        mockedVault: boolean,
    ): Promise<LinearPool> {
    ...
    static async deployedAt(address: Account): Promise<LinearPool> {
        const instance = await deployedAt(
            'v2-pool-linear/LinearPool',
            TypesConverter.toAddress(address),
        );
        const [
            poolId,
            vault,
    ...
    static getTokenList(
        ...
        tokens.push(
            mainToken.address < wrappedToken.address ? mainToken : wrappedToken,
        );
        tokens.push(
            mainToken.address < wrappedToken.address ? wrappedToken : mainToken,
        );
    ...
    get getLinearTokens(): TokenList {
        return LinearPool.getTokenList(
            this.mainToken,
            this.wrappedToken,
            this.bptToken,
        );
    ...
    getTokenIndex(token: Token): number {
        ...
        return addresses[0] == token.address
            ? 0
            : addresses[1] == token.address
            ? 1
            : 2;
    }
    ...
    async swap(params: GeneralSwap): Promise<BigNumber> {
        ...
        const { amountIn, amountOut } = expectEvent.inReceipt(
            receipt,
            'Swap',
        ).args;
    ...
    private _buildSwapParams(
        kind: number,
        params: SwapLinearPool,
    ): GeneralSwap {
        return {
            kind,
            ...
            tokenIn:
                params.in < this.tokens.length
                    ? this.tokens.get(params.in)?.address ?? ZERO_ADDRESS
                    : ZERO_ADDRESS,
            tokenOut:
                params.out < this.tokens.length
                    ? this.tokens.get(params.out)?.address ?? ZERO_ADDRESS
                    : ZERO_ADDRESS,
