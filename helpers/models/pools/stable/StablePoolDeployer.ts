...
export default {
    async deploy(params: RawStablePoolDeployment): Promise<StablePool> {
        ...
        const vaultParams = {
            ...
            mocked: params.mockedVault ?? false,
        };
        const vault =
            params?.vault ?? (await VaultDeployer.deploy(vaultParams));
        const pool = await this._deployStandalone(deployment, vault);

        const poolId = await pool.getPoolId();
        const bptIndex = await pool.getBptIndex();
        const { tokens, swapFeePercentage, amplificationParameter, owner } =
            deployment;
