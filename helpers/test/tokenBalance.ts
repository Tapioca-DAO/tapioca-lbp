...
class InternalBalanceTracker extends BalanceTracker {
    ...
    async currentBalance(): Promise<BigNumber> {
        const result = await this.vault.getInternalBalance(this.address, [
            this.token.address,
        ]);
        return result[0];
    }
...
export async function expectBalanceChange(
    ...
    const trackers: Dictionary<
        Dictionary<ERC20BalanceTracker | InternalBalanceTracker>
    > = {};
    const balanceChanges: Array<BalanceChange> = Array.isArray(balanceChange)
        ? balanceChange
        : [balanceChange];
    ...
    for (const { account, changes } of balanceChanges) {
        ...
        await tokens.asyncEach(async ({ symbol }) => {
            ...
            if (change === undefined) {
                expect(
                    delta,
                    `Expected ${delta} ${symbol} to be zero`,
                ).to.equal(0);
            } else {
                const compare: CompareFunction = Array.isArray(change)
                    ? change[0]
                    : 'equal';
