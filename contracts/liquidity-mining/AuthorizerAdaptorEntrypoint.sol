...
contract AuthorizerAdaptorEntrypoint is IAuthorizerAdaptorEntrypoint {
    ...
    function getAuthorizerAdaptor()
        public
        view
        override
        returns (IAuthorizerAdaptor)
    {
        return _adaptor;
    }
        ...
        returns (bytes memory)
    {
        ...
        bytes4 selector = data[0] |
            (bytes4(data[1]) >> 8) |
            (bytes4(data[2]) >> 16) |
            (bytes4(data[3]) >> 24);

        _require(
            canPerform(getActionId(selector), msg.sender, target),
        ...
        return
            getAuthorizerAdaptor().performAction{value: msg.value}(
                target,
                data
            );
    }
