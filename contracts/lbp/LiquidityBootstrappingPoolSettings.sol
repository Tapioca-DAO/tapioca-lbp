 ...
 */
abstract contract LiquidityBootstrappingPoolSettings is
    ...
    NewBasePool
{
    ...
    uint256 private constant _START_WEIGHT_OFFSET =
        _RECOVERY_MODE_BIT_OFFSET + 3;
    uint256 private constant _END_WEIGHT_OFFSET =
        _START_WEIGHT_OFFSET + _MAX_LBP_TOKENS * _START_WEIGHT_BIT_LENGTH;
    uint256 private constant _START_TIME_OFFSET =
        _END_WEIGHT_OFFSET + _MAX_LBP_TOKENS * _END_WEIGHT_BIT_LENGTH;
    uint256 private constant _END_TIME_OFFSET =
        _START_TIME_OFFSET + _TIMESTAMP_BIT_LENGTH;
        ...
        )
    {
        (
            ...
            ,
            endWeights
        ) = LiquidityBootstrappingPoolStorageLib.getGradualWeightUpdateParams(
            _poolState,
            _getTotalTokens()
        );
    }
    ...
    function _getNormalizedWeights() internal view returns (uint256[] memory) {
        ...
        normalizedWeights[0] = LiquidityBootstrappingPoolStorageLib
            .getNormalizedWeightByIndex(poolState, 0, pctProgress);
        normalizedWeights[1] = LiquidityBootstrappingPoolStorageLib
            .getNormalizedWeightByIndex(poolState, 1, pctProgress);
        if (totalTokens == 2) return normalizedWeights;
        normalizedWeights[2] = LiquidityBootstrappingPoolStorageLib
            .getNormalizedWeightByIndex(poolState, 2, pctProgress);
        if (totalTokens == 3) return normalizedWeights;
        normalizedWeights[3] = LiquidityBootstrappingPoolStorageLib
            .getNormalizedWeightByIndex(poolState, 3, pctProgress);
    ...
    function _setSwapEnabled(bool swapEnabled) private {
        _poolState = LiquidityBootstrappingPoolStorageLib.setSwapEnabled(
            _poolState,
            swapEnabled
        );
    ...
    function _setRecoveryMode(bool enabled) internal virtual override {
        _poolState = LiquidityBootstrappingPoolStorageLib.setRecoveryMode(
            _poolState,
            enabled
        );
    }
        ...
        returns (bool)
    {
        return
            ...
            (actionId ==
                getActionId(
                    LiquidityBootstrappingPoolSettings.setSwapEnabled.selector
                )) ||
            (actionId ==
                getActionId(
