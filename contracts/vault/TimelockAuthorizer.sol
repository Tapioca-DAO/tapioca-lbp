...
contract TimelockAuthorizer is IAuthorizer, IAuthentication, ReentrancyGuard {
    ...
    constructor(
        ...
        bytes32 grantActionId = getActionId(
            TimelockAuthorizer.grantPermissions.selector
        );
        bytes32 revokeActionId = getActionId(
            TimelockAuthorizer.revokePermissions.selector
        );
        bytes32 generalGrantActionId = getExtendedActionId(
            grantActionId,
            GENERAL_PERMISSION_SPECIFIER
        );
        bytes32 generalRevokeActionId = getExtendedActionId(
            revokeActionId,
            GENERAL_PERMISSION_SPECIFIER
        );
        ...
        SCHEDULE_DELAY_ACTION_ID = getActionId(
            TimelockAuthorizer.scheduleDelayChange.selector
        );
        ...
        returns (bytes32)
    {
        return
            keccak256(
                abi.encodePacked(bytes32(uint256(address(this))), selector)
            );
    }
     ...
     */
    function getGrantPermissionActionId(bytes32 actionId)
        public
        view
        returns (bytes32)
    {
     ...
     */
    function getRevokePermissionActionId(bytes32 actionId)
        public
        view
        returns (bytes32)
    {
     ...
     */
    function getExtendedActionId(bytes32 baseActionId, bytes32 specifier)
        public
        pure
        returns (bytes32)
    {
     ...
     */
    function getActionIdDelay(bytes32 actionId)
        external
        view
        returns (uint256)
    {
    ...
    function hasPermission(
        ...
        return
            ...
            _isPermissionGranted[
                getPermissionId(actionId, account, EVERYWHERE)
            ];
    }
    ...
    function isGranter(
        ...
        return
            _hasPermissionSpecificallyOrGenerally(
                GRANT_ACTION_ID,
                account,
                where,
                actionId
    ...
    function canGrant(
        ...
        return
            _canPerformSpecificallyOrGenerally(
                GRANT_ACTION_ID,
                account,
                where,
                actionId
    ...
    function getScheduledExecution(uint256 scheduledExecutionId)
        external
        view
        returns (ScheduledExecution memory)
    {
     ...
     */
    function isExecutor(uint256 scheduledExecutionId, address account)
        public
        view
        returns (bool)
    {
     ...
     */
    function canExecute(uint256 scheduledExecutionId)
        external
        view
        returns (bool)
    {
        ...
        ScheduledExecution storage scheduledExecution = _scheduledExecutions[
    ...
    function setDelay(bytes32 actionId, uint256 delay) external onlyExecutor {
        bytes32 setAuthorizerActionId = _vault.getActionId(
            IVault.setAuthorizer.selector
        );
        bool isAllowed = actionId == setAuthorizerActionId ||
            delay <= _delaysPerActionId[setAuthorizerActionId];
    ...
    function schedule(
        ...
        bytes32 actionId = IAuthentication(where).getActionId(
            _decodeSelector(data)
        );
        _require(
            hasPermission(actionId, msg.sender, where),
     ...
     */
    function execute(uint256 scheduledExecutionId)
        external
        nonReentrant
        returns (bytes memory result)
    {
        ...
        if (scheduledExecution.protected) {
            ...
            // scheduler.
            _require(
                isExecutor(scheduledExecutionId, msg.sender),
                Errors.SENDER_NOT_ALLOWED
            );
        }
    ...
    function cancel(uint256 scheduledExecutionId) external {
        ...
        bytes32 actionId = target.getActionId(
            _decodeSelector(scheduledExecution.data)
        );
        _require(
            hasPermission(actionId, msg.sender, scheduledExecution.where) ||
                isRoot(msg.sender),
            Errors.SENDER_NOT_ALLOWED
        );
    ...
    function manageGranter(
        ...
        (allowed ? _grantPermission : _revokePermission)(
            grantPermissionsActionId,
            account,
            where
        );
    }
    ...
    function grantPermissions(
        ...
        for (uint256 i = 0; i < actionIds.length; i++) {
            ...
            _require(
                canGrant(actionIds[i], msg.sender, where[i]),
                Errors.SENDER_NOT_ALLOWED
            );
    ...
    function scheduleGrantPermission(
        ...
        _require(
            isGranter(actionId, msg.sender, where),
            Errors.SENDER_NOT_ALLOWED
        );
        bytes memory data = abi.encodeWithSelector(
            this.grantPermissions.selector,
            _ar(actionId),
            account,
            _ar(where)
        );
    ...
    function manageRevoker(
        ...
        bytes32 revokePermissionsActionId = getRevokePermissionActionId(
            actionId
        );
        (allowed ? _grantPermission : _revokePermission)(
            revokePermissionsActionId,
            account,
            where
        );
    }
    ...
    function revokePermissions(
        ...
        for (uint256 i = 0; i < actionIds.length; i++) {
            ...
            _require(
                canRevoke(actionIds[i], msg.sender, where[i]),
                Errors.SENDER_NOT_ALLOWED
            );
    ...
    function scheduleRevokePermission(
        ...
        _require(
            isRevoker(actionId, msg.sender, where),
            Errors.SENDER_NOT_ALLOWED
        );
        bytes memory data = abi.encodeWithSelector(
            this.revokePermissions.selector,
            _ar(actionId),
            account,
            _ar(where)
        );
    ...
    function _hasPermissionSpecificallyOrGenerally(
        ...
        bytes32 generalActionId = getExtendedActionId(
            baseActionId,
            GENERAL_PERMISSION_SPECIFIER
        );
        return
            hasPermission(specificActionId, account, where) ||
            hasPermission(generalActionId, account, where);
    }
