...
contract MockVault is IPoolSwapStructs {
    ...
    constructor(IAuthorizer authorizer) {
        ...
        _protocolFeesCollector = new ProtocolFeesCollector(
            IVault(address(this))
        );
    }
    ...
    function callMinimalPoolSwap(
        ...
        uint256 amountCalculated = IMinimalSwapInfoPool(pool).onSwap(
            request,
            balanceTokenIn,
            balanceTokenOut
        );
        (uint256 amountIn, uint256 amountOut) = request.kind ==
    ...
    function callGeneralPoolSwap(
        ...
        uint256 amountCalculated = IGeneralPool(pool).onSwap(
            request,
            balances,
            indexIn,
            indexOut
